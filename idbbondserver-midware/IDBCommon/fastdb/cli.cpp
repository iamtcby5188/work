...
#ifdef USE_STD_STRING  // jialu.sun 2016/10/25
#include <iostream>
#endif
    ...
    for (cb = s->columns; cb != NULL; cb = cb->next) { 
        if (cb->get_fnc != NULL) { 
            ...
            switch (cb->var_type) { 
              ...
              case cli_asciiz:
#ifdef USE_STD_STRING  // jialu.sun 2016/10/25
                  msg_size += 4 + (((std::string*)cb->var_ptr)->length() + 1);
#else
...
#ifdef USE_STD_STRING  // jialu.sun 2016/10/25
            p = pack4(p, (int)((std::string*)(src))->length() + 1);
            const char *srcForStdstring = ((std::string*)(src))->c_str();
            while ((*p++ = *srcForStdstring++) != 0);
#else
...
int cli_clear_all_tables(int session)
{
    session_desc* s = sessions.get(session);
    if (s == NULL)
    {
    ...
    cli_request req;
    req.length = sizeof(req);
    req.cmd = cli_cmd_clear_all_tables;
    req.stmt_id = 0;
    req.pack();
    if (!s->sock->write(&req, sizeof req))
    {
...
int cli_get_database_usage(int session, cli_database_usage* db_usage)
{
    session_desc* s = sessions.get(session);
    if (s == NULL)
    {
    ...
    else if (db_usage == NULL)
    {
        return cli_bad_address;
    }
    cli_request req;
    req.length = sizeof(cli_request);
    req.cmd = cli_cmd_get_database_usage;
    req.stmt_id = 0;
    req.pack();
    if (!s->sock->write(&req, sizeof req))
    {
    ...
    int rc = unpack4(response);
    db_usage->db_size = unpack4(response+4);
    db_usage->db_used_size = unpack4(response+8);
    db_usage->db_allocated_size = unpack4(response+12);
    return rc;
}
...
int cli_send_heartbeat(int session, char const* uuid)
{
    session_desc* s = sessions.get(session);
    if (s == NULL)
    {
    ...
    else if (uuid == NULL || strlen(uuid) < 1)
    {
        return cli_bad_address;
    }
    int size = (int)(sizeof(cli_request) + strlen(uuid) + 1);
    dbSmallBuffer buf(size);
    cli_request *req = (cli_request*)buf.base();
    req->length = size;
    req->cmd = cli_cmd_send_heartbeat;
    req->stmt_id = 0;
    char* dst = (char*)(req + 1);
    strcpy(dst, uuid);
    req->pack();
