...
{
    return dbCLI::instance.clear_all_tables(session);
}

int dbCLI::clear_all_tables(int session)
{
    session_desc* s = sessions.get(session);
    if (s == NULL)
    {
...
{
    return dbCLI::instance.get_database_usage(session, db_usage);
}

int dbCLI::get_database_usage(int session, cli_database_usage* db_usage)
{
    session_desc* s = sessions.get(session);
    if (s == NULL)
    {
    ...
    db_usage->db_size = s->db->getDatabaseSize();
    db_usage->db_used_size = s->db->getUsedDatabaseSize();
    db_usage->db_allocated_size = s->db->getAllocatedSize();
    return cli_ok;
}
