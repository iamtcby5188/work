...
bool dbServer::get_database_usage(dbSession* session)
{
    char response[16];
    pack4(response, cli_ok);
    pack4(response+4, db->getDatabaseSize());
    pack4(response+8, db->getUsedDatabaseSize());
    pack4(response+12, db->getAllocatedSize());
    return session->sock->write(response, sizeof response);
}

bool dbServer::clear_all_tables(dbSession* session)
{
    dbTableDescriptor* desc = db->tables;
    if (desc == NULL)
    {
        int4 response = cli_table_not_found;
        pack4(response);
        return session->sock->write(&response, sizeof response);
    }
    else
...
bool dbServer::send_heartbeat(dbSession* session, char* data)
{
    int4 response = cli_ok;
    char* uuid = data;
    if (uuid == NULL || strlen(uuid) < 1)
    {
        response = cli_bad_address;
    }
    pack4(response);
    return session->sock->write(&response, sizeof response);
}
        ...
        while (online && session->sock->read(&req, sizeof req)) { 
            ...
            switch(req.cmd) { 
              ...
              case cli_cmd_alter_index:
                ...
                break;
              case cli_cmd_get_database_usage:
                online = get_database_usage(session);
                break;
              case cli_cmd_clear_all_tables:
                online = clear_all_tables(session);
                break;
              case cli_cmd_send_heartbeat:
                online = send_heartbeat(session, msg);
                break;
            }
        }       
