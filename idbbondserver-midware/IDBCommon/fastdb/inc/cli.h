...
/*********************************************************************
 * cli_clear_all_tables
 *     remove all tables' contents
 * Parameters:
 *     session - session descriptor as returned by cli_open
 * Returns:
 *     result code as described in cli_result_code enum
 */
int FASTDB_DLL_ENTRY cli_clear_all_tables(int session);
...
typedef struct cli_database_usage
{
    size_t db_size;
    size_t db_used_size;
    size_t db_allocated_size;
} cli_database_usage;

/**
 * cli_get_database_usage
 *     Get database usage
 * Parameters:
 *     session - session descriptor returned by cli_open
 *     db_usage - pointer to the usage structure. The folloing fields are set:
 *       db_size: database file size
 *       db_used_size: database file used size
 *       db_allocated_size: delta between size of allocated and deallocated data
 * Returns:
 *     result code as described in cli_result_code enum
 */
int FASTDB_DLL_ENTRY cli_get_database_usage(int session, cli_database_usage* db_usage);
...
*     result code as described in cli_result_code enum
*/
int FASTDB_DLL_ENTRY cli_send_heartbeat(int session, char const* uuid);
