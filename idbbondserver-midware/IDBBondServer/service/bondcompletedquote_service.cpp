...
void BondCompletedQuoteService::PublishCompletedQuoteDetailList(std::vector<std::string> bondKeyListedMarket, const char *action_type, const std::string &company_id)
{
    ...
    msg.SetString(FID_ACCOUNT_COMPANY_ID, company_id);
    sdbus::VariantList *replylist = msg.AddList(FID_LIST);

    std::vector<std::string>::const_iterator it = bondKeyListedMarket.begin();
    for (; it != bondKeyListedMarket.end(); ++it)
    {
        sdbus::VariantMap *mp = replylist->AddMap();
        mp->SetString(FID_BOND_KEY_LISTED_MARKET, *it);
    }

    PublishManager::singleton()->publish(kTopicBondCompletedQuote, "BondCompletedQuote.CompletedQuoteDetailList", msg);
}
