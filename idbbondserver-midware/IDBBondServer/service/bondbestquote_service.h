...
public:
    ...
    void PublishBondBestQuote(const std::vector<BondBestQuoteCachePtr> &bond_best_quote_vec, const char* action_type, bool without_data = true); // change default to true
	void PublishBondBestQuoteToTPRFA(const std::vector<BondBestQuoteCachePtr> &bond_best_quote_vec);
...
private:
    ...
    void GetBondBestQuoteListInc(const sdbus::Message* msg, sdbus::Message& reply);
    void GetBondCompletedQuoteListInc(const sdbus::Message* msg, sdbus::Message& reply);
