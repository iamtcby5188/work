...
BondBestQuoteCacheVecPtr BondBestQuoteBusiness::FetchBondBestQuotes(const sdbus::string& cond, const sdbus::string& sortby, const sdbus::string& range)
{
    ...
    BondBestQuoteCacheVecPtr best_quotes = bondBestQuoteCC.getCacheListByQueryInThreadSafty(query);
    std::vector<BondBestQuoteCachePtr>::iterator it = best_quotes->begin();
    for ( ; it != best_quotes->end(); ++it) {
        BondBestQuoteCachePtr bond_best_quote = *it;
        if (bond_best_quote) {
            ChinaBondValuationCacheController china_bond_valuation_cc;
            ChinaBondValuationCachePtr china_bond_valuation_cache = china_bond_valuation_cc.getValuation(bond_best_quote->bond_id);

            if (china_bond_valuation_cache) {
                bond_best_quote->cdc_valuation_yield = china_bond_valuation_cache->valuation_yield;
                bond_best_quote->cdc_valuation_net_price = china_bond_valuation_cache->valuation_net_price;
                if (bond_best_quote->cdc_valuation_yield.length() > 0) {
                    if (bond_best_quote->ofr_yield != DOUBLE_NULL) {
                        bond_best_quote->cdc_offset = (bond_best_quote->ofr_yield - atof(bond_best_quote->cdc_valuation_yield.c_str())) * 100;
                    } else if (bond_best_quote->ofr_price != DOUBLE_NULL && bond_best_quote->ofr_quote_type == "0") {
                        bond_best_quote->cdc_offset = (bond_best_quote->ofr_price - atof(bond_best_quote->cdc_valuation_yield.c_str())) * 100;
                    }
                } 
            } 
        }
...
sdbus::string BondBestQuoteBusiness::CopyBestQuoteAll(sdbus::VariantList quote_ids_all) 
{
    ...
    for (size_t i = 0; i < quote_ids_all.Size(); ++i){
...
sdbus::string BondBestQuoteBusiness::CopyBestQuoteForTP(sdbus::VariantList quote_ids) 
{
	...
	for (size_t i = 0; i < quote_ids.Size(); ++i)
    {
		...
		if (best_quote_cache->bid_offer_id != "")
        {
            ...
            {
                for (size_t i = 0; i < bid_quotes_list.size(); ++i)
                {
		...
		if (best_quote_cache->ofr_offer_id != "")
        {
            ...
            {
                for (size_t i = 0; i < ofr_quotes_list.size(); ++i)
                {
...
sdbus::string BondBestQuoteBusiness::CopyInternalBestQuoteForTP(sdbus::VariantList quote_ids) 
{
    ...
    {        
        ...
        // [Huameng, 2015-09-14] 操作后导出的最优报价（暗盘）不包含无报价的券。注意此处和IDBBondServer下的导出不一样!!
