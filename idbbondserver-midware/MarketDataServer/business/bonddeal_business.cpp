...
BondDealCacheVecPtr BondDealBusiness::FetchBondDeals(const sdbus::string& cond, const sdbus::string& sortby, const sdbus::string& range)
{
    ...
    BondDealCacheVecPtr deals = bondDealCC.getCacheListByQueryInThreadSafty(query);
    std::vector<BondDealCachePtr>::iterator it = deals->begin();
    for ( ; it != deals->end(); ++it) {
        BondDealCachePtr bond_deal = *it;
        if (bond_deal) {
...
BondDealCacheVecPtr BondDealBusiness::FetchBondDealsByDate(const std::string& cond, const std::string& datetime)
{
    ...
    std::string qStr = " bond_category = '" + cond + "'" + GetCompanyConditionStringWithPreAnd() + " and deal_time like '" + str + "' order by bond_subtype,coupon_type desc";
...
//------------------------------------------------------------------------------------------------------------------------
int BondDealBusiness::GetDealSequenceNumber()
{
    ...
    BondDealCacheController bondDealCC;
    BondDealCacheVecPtr deal_cache_vec = bondDealCC.getCacheListByQueryInThreadSafty(query);
