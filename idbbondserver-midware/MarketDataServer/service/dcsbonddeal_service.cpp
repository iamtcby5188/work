...
#include "config.h"
#include "datamanager/dcs_deal_data_manager.h"
...
void DcsBondDealService::processReqMsg(const sdbus::Message* msg)
{
    ...
    if (method == "DcsBondDeal.DealList") {
        if (subject == "model") {
            ...
            reply.SetInt32(FID_PUSH_MESSAGE_VERSION, DcsDealDataManager::GetVersion(GetCurrentCompanyId(msg)));
        }
...
void DcsBondDealService::SetBondDealCache(const sdbus::VariantMap *mp, DcsBondDealCache *cache)
{
	...
	mp->GetString(FID_DCS_QUESTION_MARK, cache->questionMark);
	...
	mp->GetString(FID_OLD_CONTRACT_ID, cache->oldContractId);
	mp->GetString(FID_RELATED_MSG, cache->relatedMsg);
    ...
    mp->GetString(FID_DCS_DESTROY_REASON, cache->destroy_reason);
}
...
void DcsBondDealService::GetBondDealList(const sdbus::Message* msg, sdbus::Message& reply)
{
	...
	{
		DcsBondDealCache* bond_deal = deals->at(i).get();
